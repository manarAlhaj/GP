...
from preprocessing import preprocess_single, compute_signer_stats

TARGET_T     = 100   # fixed by normalize_length() in preprocess_single()
NUM_FEATURES = 16    # fixed by preprocessing pipeline (8 relational + 8 delta)

# ══════════════════════════════════════════════════════════════
# COMPUTE Z-SCORE STATS FROM TRAINING DATA
# ══════════════════════════════════════════════════════════════

def compute_global_stats(train_df, target_length=100):
    """
    Loads and preprocesses every training recording.
    Computes mean and std for z-score normalization.

    Must be called on training data ONLY — never on validation.
    """
    all_processed = []
    ...
    for _, row in train_df.iterrows():
        ...
        arr = preprocess_single(arr, target_length)          # (100, 16)
        all_processed.append(arr)

    mean, std = compute_signer_stats(all_processed)          # (16,), (16,)
...
def main():
   ...
   # target_T = PickTargetT(train_df, percentile=95.0)
    ...
    print(f"Target timesteps  : {TARGET_T}  (fixed by normalize_length)")
    print(f"Feature count     : {NUM_FEATURES}  (fixed by preprocessing pipeline)")

    global_mean, global_std = compute_global_stats(train_df, TARGET_T)

# Save for Raspberry Pi inference
   ...
   # num_features = sample_arr.shape[1]
    ...
    model = Lstm(
        featuresNum=NUM_FEATURES, #16
        classesNum=num_classes,
        TimeSteps=TARGET_T, #100
        units1=args.lstm1,
        units2=args.lstm2,
        dense=args.dense,
        l2=args.l2,
